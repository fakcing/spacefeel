...
export default async function ActorPage({ params }: Props) {
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="relative px-4 md:px-8 max-w-7xl mx-auto">
          <div className="flex flex-col sm:flex-row gap-6 sm:gap-8">
            <div className="flex-shrink-0 flex justify-center sm:block">
              <div className="relative w-32 h-32 sm:w-44 sm:h-44 rounded-2xl overflow-hidden" style={{ backgroundColor: 'var(--color-overlay)', border: '1px solid var(--color-border)' }}>
                {avatarUrl ? (
                  <Image src={avatarUrl} alt={p.name} fill className="object-cover" sizes="176px" priority />
                ) : (
                  <div className="w-full h-full flex items-center justify-center">
                    <Film size={40} style={{ color: 'var(--color-text-subtle)' }} />
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex-1 min-w-0 text-center sm:text-left">
              ...
              <div className="flex flex-wrap gap-4 mb-4 justify-center sm:justify-start">
                {p.birthday && (
                  <div className="flex items-center gap-1.5 text-sm" style={{ color: 'var(--color-text-muted)' }}>
                    ...
                    {p.deathday && <span>— {formatDate(p.deathday)}</span>}
                  </div>
                )}
                {p.place_of_birth && (
                  <div className="flex items-center gap-1.5 text-sm" style={{ color: 'var(--color-text-muted)' }}>
              ...
              {p.biography && (
                <p className="text-sm leading-relaxed line-clamp-5 max-w-2xl" style={{ color: 'var(--color-text-muted)' }}>
                  {p.biography}
                </p>
              )}
            </div>
      ...
      <div className="px-4 md:px-8 max-w-7xl mx-auto pb-24 space-y-10">
        {movies.length > 0 && (
          <section>
                ...
                return (
                  <Link key={`movie-${item.id}`} href={href} className="group">
                    <div className="aspect-[2/3] rounded-xl overflow-hidden mb-1.5" style={{ backgroundColor: 'var(--color-overlay)' }}>
                      {poster ? (
                        <Image src={poster} alt={title} width={140} height={210} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center">
                    ...
                    <p className="text-xs font-medium line-clamp-2 leading-tight" style={{ color: 'var(--color-text)' }}>{title}</p>
                    {year && <p className="text-[10px] mt-0.5" style={{ color: 'var(--color-text-subtle)' }}>{year}</p>}
                  </Link>
                )
              })}
            </div>
                  ...
                  <Link key={`tv-${item.id}`} href={href} className="group">
                    <div className="aspect-[2/3] rounded-xl overflow-hidden mb-1.5" style={{ backgroundColor: 'var(--color-overlay)' }}>
                      {poster ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          <Tv size={20} style={{ color: 'var(--color-text-subtle)' }} />
                        </div>
                      )}
                    </div>
                    <p className="text-xs font-medium line-clamp-2 leading-tight" style={{ color: 'var(--color-text)' }}>{title}</p>
