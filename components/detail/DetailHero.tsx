...
import { Play, Plus, BookmarkCheck, RotateCcw, Star } from 'lucide-react'
  ...
  return (
    <>
      <div className="relative w-full h-[50vh] sm:h-[60vh] overflow-hidden">
        {backdrop ? (
          <Image src={backdrop} alt={title || ''} fill className="object-cover" priority quality={85} sizes="100vw" />
        ) : (
          <div className="absolute inset-0" style={{ backgroundColor: 'var(--color-overlay)' }} />
        )}
        <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, var(--color-bg) 0%, rgba(0,0,0,0.45) 50%, transparent 100%)' }} />
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 -mt-20 sm:-mt-28 relative z-10 flex flex-col sm:flex-row gap-6 sm:gap-8 items-start sm:items-end pb-6 sm:pb-10">
        ...
        <div className="sm:hidden w-full flex justify-center">
          <div className="w-36 rounded-xl overflow-hidden flex-shrink-0" style={{ boxShadow: '0 20px 60px rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.08)' }}>
            <div className="relative aspect-[2/3]">
              {poster && <Image src={poster} alt={title || ''} fill className="object-cover" priority sizes="144px" />}
            </div>
          </div>
        </div>
        ...
        <div className="hidden sm:block flex-shrink-0 w-44 rounded-2xl overflow-hidden" style={{ boxShadow: '0 24px 64px rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.08)' }}>
          <div className="relative aspect-[2/3]">
            {poster && <Image src={poster} alt={title || ''} fill className="object-cover" priority sizes="176px" />}
          </div>
        </div>
          ...
          <div className="flex items-center justify-center sm:justify-start gap-4 flex-wrap mb-5">
            {item.vote_average > 0 && (
              <span className="flex items-center gap-1 text-sm font-semibold text-amber-400">
                ...
                {item.vote_count > 0 && (
                  <span className="text-xs font-normal ml-0.5" style={{ color: 'var(--color-text-subtle)' }}>
                    ({item.vote_count.toLocaleString('en-US')})
                  </span>
                )}
              </span>
            )}
